<template>
  ...
  >
    ...
    <template #toolbar>
      <el-button
        type="primary"
        @click="$router.push({ name: 'authPermissionAdd' })"
      >
    ...
    <template #module_status="status">
      <span :style="{ color: status.row.status === 1 ? 'red' : '' }">
        {{ status.row.status === 1 ? '停用' : '正常' }}
      </span>
    </template>
    ...
    <el-form ref="editForm" :model="currentData" label-width="90px">
      ...
      <el-form-item label="模块状态">
        <el-radio-group v-model="currentData.status">
          <el-radio :label="0" :checked="currentData.status === 0">
            正常
          </el-radio>
          <el-radio :label="1" :checked="currentData.status === 1">
            停用
          </el-radio>
        </el-radio-group>
      </el-form-item>
  ...
  setup() {
    ...
    const state = reactive({
      columns: [
        ...
        { label: 'ID', prop: 'id' },
        { label: '标题', prop: 'title' },
        { label: '点击次数', prop: 'click' },
        { label: '状态', prop: 'status', tdSlot: 'module_status' },
